...
module.exports = {
    ...
    findEmployees: function (hospitalId, page, conditions) {
        ...
        if (conditions.length) {
            sql = 'select SQL_CALC_FOUND_ROWS e.id, e.`name`, d.`name` as department, e.mobile, e.gender, e.birthday, job.`name` as jobTitle, role.`name` as role, e.`status`, e.maxDiscountRate  from Employee e LEFT JOIN Department d on d.id = e.department left JOIN Role role on role.id = e.role left JOIN JobTitle job on job.id = e.jobTitle where e.`status` <> 2 and e.hospitalId =? and ' + conditions.join(' and ') + ' order by e.id desc limit ?,?';
        }
    ...
    findDoctorsByHospital: function (hospital, page, conditions) {
        ...
        if (conditions.length) sql = 'select SQL_CALC_FOUND_ROWS d.*, e.birthday, d.clinic, e.mobile from Doctor d, Employee e where e.id = d.employeeId and d.status <> 2 and d.hospitalId = ? and ' + conditions.join(' and ') + ' order by d.createDate limit ?, ?';
